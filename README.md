...
In order to avoid a poorly organized `rc.lua` spanning thousands of lines, it has been split into multiple files / folders. It has a logical directory structure that will hopefully allow those new to awesomewm to have an easy time navigating it. (Or die trying!)
...
+ This repo is a fork from [here](https://github.com/WillPower3309/awesome-dotfiles). The original one is cooler, so check it out :)
...
+ If you would like to change the wallpaper, ensure that the wallpaper is named "wallpaper" and is located in the `~/.config/awesome/wallpaper` folder.
